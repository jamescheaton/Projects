...
static packet_t *packetize(char *buffer, int length, int *count) {
	...
	size_t arr_length = (size_t)(length / MAX_PAYLOAD_LENGTH);
	if((size_t)length > MAX_PAYLOAD_LENGTH * arr_length)
	{
		arr_length++;
	}
	...
	*count = 1;
	int i;
    packet_t *packets = calloc(arr_length, sizeof(packet_t));
	while(length > (*count) * MAX_PAYLOAD_LENGTH)
	{
		i = *count - 1;
		packet_t *current = packets + i;
		current->payload_length = MAX_PAYLOAD_LENGTH;
		current->type = DATA;
		
		memcpy(current->payload, (char*)(buffer + (i * MAX_PAYLOAD_LENGTH * (int)sizeof(char))), MAX_PAYLOAD_LENGTH * sizeof(char));
		int check = checksum((char*)&current->payload, MAX_PAYLOAD_LENGTH);
		current->checksum = check;
		//buffer = buffer + MAX_PAYLOAD_LENGTH;

		current->payload_length = MAX_PAYLOAD_LENGTH;
		*count = *count + 1;
	}
	i = *count - 1;
	packet_t *last = packets + i;
	last->type = LAST_DATA;
	last->payload_length = length % MAX_PAYLOAD_LENGTH;
	memcpy(last->payload, (char*)buffer + i * MAX_PAYLOAD_LENGTH * (int)sizeof(char), (size_t)last->payload_length * sizeof(char));
	int check = checksum((char*)last->payload, last->payload_length);
	last->checksum = check;
...
static void *rtp_recv_thread(void *void_ptr) {
    ...
    do {
        ...
        int buffer_length = 0;
        packet_t packet;
		size_t current_max = 100;
		char *buffer = malloc(current_max);
        ...
        do {
            ...
            */
			...
			if((size_t)(buffer_length + MAX_PAYLOAD_LENGTH) > current_max)
			{
			...
			if(type == DATA)
			{
				int header_checksum = packet.checksum;
				int length = packet.payload_length;
				int packet_checksum = checksum(packet.payload, length);

				if(header_checksum == packet_checksum)
				{
					packet_t send_ack;
					send_ack.type = ACK;
					send_ack.checksum = 0;
					send_ack.payload_length = 0;
					//send_ack->payload = 0;
					net_send_packet(connection->net_connection_handle, &send_ack);
					memcpy(buffer + buffer_length, packet.payload, (size_t)length * sizeof(char));
					buffer_length = buffer_length + MAX_PAYLOAD_LENGTH;

				} else {
					packet_t send_nack;
					send_nack.type = NACK;
					send_nack.checksum = 0;
					send_nack.payload_length = 0;
					//send_nack->payload = 0;
					net_send_packet(connection->net_connection_handle, &send_nack);
				}
			} else if(type == LAST_DATA)
			{
				int header_checksum = packet.checksum;
				int length = packet.payload_length;
				int packet_checksum = checksum(packet.payload, length);
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
             ...
             */
			pthread_mutex_lock(&connection->ack_mutex);
			while(connection->nack != 0 && connection->nack != 1)
			{
				pthread_cond_wait(&connection->ack_cond, &connection->ack_mutex);
			}
			if(connection->nack == 1)
			{
				i--;
			}
			connection->nack = 2;
