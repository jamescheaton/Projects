...
void system_init(void) {
     ...
     */
	 ...
	 frame_table = (fte_t*)mem;	// frame table pointer to first frame table
	 memset(frame_table, 0, PAGE_SIZE); // zero out memory used by page table
	 frame_table->protected = 1; // set protected bit
...
void proc_init(pcb_t *proc) {
     ...
     */
	 ...
	 pfn_t freed_frame = free_frame(); // get free frame
	 memset(mem + (freed_frame * PAGE_SIZE), 0, PAGE_SIZE); // zero out memory in frame
	 proc->saved_ptbr = freed_frame; // update the PCB



	 (frame_table + freed_frame)->protected = 1; // set protected bit
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
	...
	pfn_t pfn = pte->pfn;
	fte_t *current_frame = frame_table + pfn;
