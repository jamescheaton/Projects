...
*/
void compute_stats() {
	uint64_t num_access = stats.accesses;
	uint64_t num_disk_reads = stats.page_faults;
	uint64_t num_disk_writes = stats.writebacks;
	double average = (num_access * MEMORY_READ_TIME) + (num_disk_reads * DISK_PAGE_READ_TIME) + (num_disk_writes * DISK_PAGE_WRITE_TIME);
	average = average/num_access;
	stats.aat = average;
}
